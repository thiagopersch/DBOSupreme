...
Panels.Buff = function(parent)
    context.macro(1000, "Buff Spell", nil, function()
        if not context.hasBuff() and context.storage.buffText:len() > 0 then
            if context.saySpell(context.storage.buffText, 2500) then
                context.delay(30000)
            end
        end
    end, parent)
    context.addTextEdit("buffText", context.storage.buffText or "Power UP", function(widget, text) context.storage.buffText = text end, parent)
end
