...
local function gotStatus(data, err)
    if err then return updateError(err) end
    ...
    statusData = data
    if checksumIter == 100 then compareChecksums() end
end
...
function Updater.show()
    if not g_resources.isLoadedFromArchive() or Services.updater == nil or
        Services.updater:len() < 4 then return Updater.hide() end
    if updaterWindow:isVisible() then return end
    updaterWindow:show()
    updaterWindow:raise()
    updaterWindow:focus()
...
function Updater.isVisible() return updaterWindow:isVisible() end
...
function Updater.hide()
    ...
    if thingsUpdateOptionalError then
        local msgbox = displayErrorBox("Updater error",
                                       thingsUpdateOptionalError:trim())
...
function generateChecksum()
    ...
    if checksumIter == 99 then toEntry = #updateableFiles end
...
function gotChecksums() if statusData ~= nil then compareChecksums() end end
...
function compareChecksums()
    ...
    if #toUpdate == 0 then return upToDate() end
    -- outdated
    filesUrl = statusData["url"]
    initialPanel:hide()
    updatePanel:show()
    updatePanel:getChildById('updateStatusLabel'):setText(
        tr("Updating %i files", #toUpdate))
    updaterWindow:setHeight(190)
    downloadNextFile(false)
end

function upToDate() Updater.hide() end
...
function urlencode(url)
    url = url:gsub("\n", "\r\n")
    url = url:gsub("([^%w ])",
                   function(c) string.format("%%%02X", string.byte(c)) end)
    url = url:gsub(" ", "+")
    return url
end
...
function downloadNextFile(retry)
    ...
    if downloadIter == #toUpdate then return downloadingFinished() end
    ...
    downloadStatusLabel:setText(tr("Downloading %i of %i%s:\n%s",
                                   downloadIter + 1, #toUpdate, retry, file))
    downloadProgressBar:setPercent(0)
    downloadProgressBar:setText("")
    HTTP.download(filesUrl .. urlencode(file), file, onDownload,
                  onDownloadProgress)
end

function downloadingFinished()
    thingsUpdateOptionalError = nil
    UIMessageBox.display(tr("Success"),
                         tr("Download complate.\nUpdating client..."), {}, nil,
                         nil)
